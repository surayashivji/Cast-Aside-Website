...
function SiriWave(opt) {
	...
	if (opt.cover) {
		...
		this.debounceResize = debounce(function(ev) {
			self.width = opt.width || window.getComputedStyle(self.container).width.replace('px', '');
			self.height = opt.height || window.getComputedStyle(self.container).height.replace('px', '');
			self.ratio = opt.ratio || window.devicePixelRatio || 1;
			self.cache.width = self.ratio * self.width;
			self.cache.height = self.ratio * self.height;
			self.cache.height2 = self.cache.height / 2;
			self.cache.width2 = self.cache.width / 2;
			self.cache.width4 = self.cache.width / 4;
			self.cache.heightMax = (self.cache.height2) - 4;
			self.canvas.width = self.cache.width;
			self.canvas.height = self.cache.height;
			self.canvas.style.width = (self.cache.width / self.ratio) + 'px';
			self.canvas.style.height = (self.cache.height / self.ratio) + 'px';
		}, 10);
	...
	this.container.insertBefore(this.canvas, this.container.firstElementChild);
	if (opt.autostart) {
		this.start();
	}
}
